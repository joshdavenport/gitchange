...
#[test]
fn the_repo_root_named_as_a_path_is_the_directory_refusal() {
    // The root strips to the empty path, which is the same refusal `sub`
    // gets — and the case where both sides of the comparison are the whole
    // worktree, so a spelling divergence would show up as an escape (#181).
    let repo = owned_repo();

    let stderr = refusal(repo.path(), &["--", "."]);

    assert!(
        stderr.contains("'.' is a directory — name the changed files under it:"),
        "{stderr}"
    );
}
