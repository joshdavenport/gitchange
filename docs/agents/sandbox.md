...
| `mixed-mode` | Issue #101's two corners: `scripts/release.sh` with a worktree chmod (`○` mode hunk, filed under `permissions`) beside the staged-then-reverted `◑` edit under `release-tooling`, and `scripts/bench.sh` with a staged chmod (`●`) beside an unstaged edit — `◐ 1/2`, never wholly `○`. The mode hunk is a selectable diff row, first, and reads foreign and dimmed from `release-tooling`. Mode hunks show on unix only. |
