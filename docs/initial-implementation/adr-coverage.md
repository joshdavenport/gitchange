...
| CLI shell-out fallback for apply edge cases | **Recorded deviation** — trigger covered by `apply_corpus.rs` (green = untriggered) | Not implemented, and re-scoped by the ADR from deferred work to a mitigation conditional on an observed `Error::ApplyFailed`. Trigger variant exists and is mapped from the libgit2 `apply` call alone; the corpus is its certification suite. Held in issue #55. |
