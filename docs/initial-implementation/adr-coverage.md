...
| Detached HEAD: pin, no commit guard | Pin: `app.rs::conditions_pin_and_self_clear`; reporting: `refresh.rs::snapshot_head_reports_detached_by_short_id`; absent guard: `operations.rs::a_detached_head_is_not_an_operation_and_leaves_commit_unguarded` (#61) | The absence is now pinned: `operation` reads `None` while detached, the commit succeeds with its payload in HEAD, and HEAD is still detached (at the new commit) afterwards, so widening `operation()` to cover detached HEAD fails a test. |
